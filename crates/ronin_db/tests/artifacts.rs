...
fn artifact_crud_round_trip() {
    ...
    let msg = db.create_message(&thread.id, "user", "create artifact", "complete").expect("create message");

    // 1. Create
    let artifact = db.create_artifact(
        &thread.id,
        &msg.id,
        "My Artifact",
        "some content"
    ...
    // 2. Get by ID
    let fetched = db.get_artifact(&artifact.id).expect("get artifact").expect("artifact should exist");
    assert_eq!(fetched, artifact);

    // 3. List by thread
    let artifacts = db.list_artifacts_for_thread(&thread.id).expect("list artifacts");
    assert_eq!(artifacts.len(), 1);
    assert_eq!(artifacts[0], artifact);

    // 4. Delete
    db.delete_artifact(&artifact.id).expect("delete artifact");
    let fetched_after_delete = db.get_artifact(&artifact.id).expect("get artifact after delete");
